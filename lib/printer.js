...
var getNativeLibPath = function(){
    return binary.find(path.resolve(path.join(__dirname,'../package.json')));
};
var native_lib_path = getNativeLibPath();
if(!fs.existsSync(native_lib_path)) {
    native_lib_path = path.join(__dirname, 'native','node_printer.node');
}
printer_helper = require(native_lib_path);

module.exports.getNativeLibPath = getNativeLibPath; 
